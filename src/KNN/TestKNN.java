...
public class TestKNN {
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            for (int i = 0; i < testDatas.size(); i++) {
                List<Double> newDatas = testDatas.get(i);
                //System.out.println(Math.round(Float.parseFloat((knn.knn(trainDatas, newDatas, 3)))));
                result.add(Double.parseDouble((knn.knn(trainDatas, newDatas, 10))));
            }
            ...
            for(int i = 0; i <testDatas.size(); i++){
                List<Double> row = testDatas.get(i);
                if(row.get(0).equals(result.get(i))){
                    count++;
                }
            }
